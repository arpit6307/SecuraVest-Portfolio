...
const ReviewSubmissionForm = () => {
    ...
    // --- NEW LOGIC: Determine Sentiment Tag ---
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        // NOTE: alert() is used here only in a non-critical validation path, 
        // ideally replaced by a custom modal in a full production app.
        if (!formData.policyType) { alert("Please select a policy type."); return; }
    ...
    return (
        <div className="glass-form-card h-100 position-relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="position-relative z-2">
                ...
                {status === 'success' && <div className="mt-3 text-success small fw-bold text-center"><FaCheckCircle className="me-1"/> Submitted for Approval!</div>}
                {status === 'error' && <div className="mt-3 text-danger small fw-bold text-center"><FaExclamationCircle className="me-1"/> Failed. Try again.</div>}
